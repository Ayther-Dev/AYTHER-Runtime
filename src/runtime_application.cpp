    ...
    while (running) {
        ...
        if (swap_ok && renderer_ok && fv.fb_pixels) {
            if (auto acquired_frame = swapchain.begin_frame(vulkan);
                ...
                // Acquire can fail because the surface became out-of-date or
                // because recording could not start. Replace the whole frame
                // state before trying again; never reuse an acquired image.
                int w, h;
                SDL_GetWindowSizeInPixels(window, &w, &h);
                (void)rebuild_presentation(w, h);
            }
