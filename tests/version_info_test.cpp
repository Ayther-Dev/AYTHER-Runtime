...
int main() {
    ...
    check(ayther::runtime::runtime_version == "0.1.0",
          "Runtime reports the CMake project version");
    ...
    check(ayther::runtime::version_report() ==
              "AYTHER Runtime 0.1.0; Engine " + expected_engine,
          "the startup log reports both component versions");

    const std::string title =
        ayther::runtime::window_title("Vulkan Passthrough");
    check(title == "AYTHER Runtime 0.1.0 — Engine " + expected_engine +
                       " — Vulkan Passthrough",
