...
> [!WARNING]
> **Beta:** product version `0.1.0-beta.5` is an internal evaluation release.
...
- CMake 3.21 or newer and a C++20 compiler. On Windows, the published
  `v0.1.0-rc.9` archive supports either Microsoft `cl` or LLVM `clang-cl` as
...
and exact `rc.9` tag, extracts the package below `.deps/`, and returns its
...
or an ABI/link failure stops configuration with the detected values, the rc.9
